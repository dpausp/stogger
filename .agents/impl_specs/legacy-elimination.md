...
- All ADRs: remove `<!-- Tests will be added after implementation -->` placeholder comments
...
- CODEX Article 4: DEATH TO ALL LEGACY AND COMPATIBILITY
- ADR `format-config-extension`: documents dead fields in ConsoleFileRenderer
- ADR `stogger-systemd`: documents journal integration (systemd was later absorbed as built-in module)
- ADR `stogger-self-logging`: documents commented-out infrastructure_files removal
- Impl spec `raw-output`: decision `skip-list-cleanup` that kept bare `"output"` for backward compat
...
description: "Remove dead code, broad exception handling, backwards-compat artifacts, stale documentation, and lifecycle debt from stogger codebase"
created_at: "2026-05-23T18:00:00Z"
git_rev: d94a27e
target_tests:
  - file: tests/impl_spec/test_legacy_elimination.py
    tests:
      - TestDeadCodeRemoval::test_regexes_module_deleted
      - TestDeadCodeRemoval::test_journal_logger_factory_not_in_core
      - TestDeadCodeRemoval::test_journal_logger_factory_not_exported
      - TestDeadCodeRemoval::test_no_dead_fields_show_logger_brackets
      - TestDeadCodeRemoval::test_no_dead_fields_show_pid
      - TestDeadCodeRemoval::test_no_bare_isatty_call
      - TestBroadExceptionCleanup::test_init_early_logging_no_bare_except
      - TestBroadExceptionCleanup::test_multi_renderer_reraises
      - TestBroadExceptionCleanup::test_multi_optimistic_logger_reraises
      - TestBroadExceptionCleanup::test_load_pyproject_config_no_bare_exception
      - TestBroadExceptionCleanup::test_no_pending_warnings_global
      - TestBroadExceptionCleanup::test_no_atexit_registration
      - TestBackwardsCompatPurge::test_no_bare_output_in_skip_list
      - TestBackwardsCompatPurge::test_log_to_stdlib_removed
      - TestBackwardsCompatPurge::test_no_bumpversion_in_pyproject
      - TestBackwardsCompatPurge::test_release_script_no_stale_projects
      - TestVultureWhitelistCleanup::test_vulture_whitelist_has_no_stale_entries
      - TestStaleDocumentationFix::test_conventions_no_mypy_reference
      - TestStaleDocumentationFix::test_type_checking_guide_correct_import
      - TestStaleDocumentationFix::test_adrs_no_placeholder_comments
      - TestStaleDocumentationFix::test_pyproject_no_commented_infrastructure_files
      - TestSpecTestLifecycleCleanup::test_impl_spec_decorators_docs_deleted
      - TestSpecTestLifecycleCleanup::test_impl_spec_format_config_deleted
      - TestSpecTestLifecycleCleanup::test_impl_spec_postgres_deleted
      - TestColoramaAvailabilityCheck::test_colors_no_colorama_fallback
      - TestOptionalImportSimplification::test_systemd_import_is_direct
```
