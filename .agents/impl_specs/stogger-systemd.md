...
- packages/stogger-systemd/pyproject.toml — hatch-vcs with raw-options.root = "../..", deps: stogger + systemd-python, Linux classifier
- packages/stogger-systemd/src/stogger_systemd/__init__.py — exports get_journal_logger_factory, JournalLogger, DummyJournalLogger, JournalLoggerFactory
- tests/test_systemd_integration.py — 4 test cases with mocked imports

### Files to modify

- src/stogger/core.py — init_logging(): add StoggerConfig read, journal dynamic import block with ImportError fallback, JOURNAL_STREAM info message, facility plumbing (read from config, pass through to renderer)
- pyproject.toml — add [tool.uv.workspace] members = ["packages/*"], add [tool.uv.sources] for stogger-systemd workspace member

### Acceptance criteria

1. pip install stogger-systemd on Linux pulls stogger + systemd-python
2. init_logging() sends to journal via journal.send() when stogger-systemd installed
3. init_logging() falls back gracefully when stogger-systemd absent
