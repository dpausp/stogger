...
def generate_test_logs():
    ...
    log.info("demo_started", 
             demo_version="1.0",
             timestamp=time.time(),
             user="demo-user")
    
    log.debug("configuration_loaded",
              config_file="/etc/demo/config.yaml",
              settings_count=42)
    
    log.warning("memory_usage_high",
                memory_percent=85.5,
                threshold=80,
                process="demo-app")
...
def show_mock_output():
    ...
    from nicestlog.core import RESET_ALL, BRIGHT, DIM, RED, BLUE, CYAN, MAGENTA, YELLOW, GREEN
    
    mock_entries = [
        ("14:23:45.123", "I", "journal-viewer-demo", "1234", "demo_started", 
         {"demo_version": "1.0", "user": "demo-user"}),
        ("14:23:45.156", "D", "journal-viewer-demo", "1234", "configuration_loaded",
         {"config_file": "/etc/demo/config.yaml", "settings_count": 42}),
        ("14:23:45.189", "W", "journal-viewer-demo", "1234", "memory_usage_high",
         {"memory_percent": 85.5, "threshold": 80}),
        ("14:23:45.234", "E", "journal-viewer-demo", "1234", "database_connection_failed",
         {"host": "db.example.com", "error": "Connection timeout"}),
        ("14:23:45.267", "C", "journal-viewer-demo", "1234", "system_overload",
    ...
    for timestamp, level, service, pid, message, fields in mock_entries:
        ...
        if fields:
            ...
            for key, value in fields.items():
                field_parts.append(f"{CYAN}{key}{RESET_ALL}={MAGENTA}{value}{RESET_ALL}")
...
def show_comparison():
    ...
    print("Dec 07 14:23:45 hostname myapp[1234]: {'level': 'info', 'event': 'user_login', 'user_id': 12345, 'timestamp': '2023-12-07T14:23:45.123456'}")
    print("Dec 07 14:23:46 hostname myapp[1234]: {'level': 'error', 'event': 'db_error', 'error': 'connection failed', 'host': 'db.example.com'}")
    
    print("\n😍 Beautiful nicestlog journal output:")
    print("14:23:45.123 I [myapp] (1234) user_login user_id=12345")
    print("14:23:46.789 E [myapp] (1234) db_error error='connection failed' host=db.example.com")
