...
class LogQualityReviewer:
    ...
    def _find_issues(self, stats: Dict[str, Any]) -> List[str]:
        ...
        if stats['debug_prints'] > 0:
            issues.append(f"🚫 {stats['debug_prints']} Debug-Prints gefunden - des ghört weg!")
    ...
    def _generate_suggestions(self, stats: Dict[str, Any]) -> List[str]:
        ...
        if stats['structured_lines'] < stats['total_lines'] * 0.7:
            suggestions.append("💡 Mehr strukturierte Logs verwenden (JSON oder key=value)")
        
        if len(stats['levels_found']) < 3:
            suggestions.append("💡 Mehr Log-Level verwenden (DEBUG, INFO, WARNING, ERROR)")
        ...
        if 'user_id' not in stats['fields_found']:
            suggestions.append("💡 User-Kontext zu Logs hinzufügen (user_id, session_id)")
