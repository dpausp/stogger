...
def check_logging_quality(stats: LoggingStats, min_coverage: float = 5.0, max_coverage: float = 15.0) -> List[str]:
    ...
    if stats.log_coverage_percent < min_coverage:
        issues.append(f"❌ Too little logging! {stats.log_coverage_percent:.1f}% coverage (minimum: {min_coverage}%)")
        issues.append(f"   Add more log.info(), log.debug(), or log.error() statements")
    elif stats.log_coverage_percent > max_coverage:
        issues.append(f"⚠️  Possibly too much logging! {stats.log_coverage_percent:.1f}% coverage (maximum: {max_coverage}%)")
    ...
    if stats.functions > 0:
        if stats.function_coverage_percent < 30:
            issues.append(f"❌ Too few functions have logging: {stats.function_coverage_percent:.1f}%")
            issues.append(f"   Consider adding logging to more functions (aim for 30-70%)")
        elif stats.function_coverage_percent > 90:
            issues.append(f"⚠️  Almost every function logs - might be excessive: {stats.function_coverage_percent:.1f}%")
        else:
            issues.append(f"✅ Good function logging coverage: {stats.function_coverage_percent:.1f}%")
    
    return issues


def lint_directory(directory: Path, min_coverage: float = 5.0, max_coverage: float = 15.0) -> bool:
    ...
    for file_path in python_files:
        ...
        if any('❌' in issue or '⚠️' in issue for issue in issues):
            total_issues += 1
            print(f"📁 {file_path}")
            print(f"   Lines: {stats.total_lines}, Code: {stats.code_lines}, Logs: {stats.log_statements}")
            for issue in issues:
                print(f"   {issue}")
            print()
    ...
    if total_stats.code_lines > 0:
        ...
        total_stats.function_coverage_percent = (total_stats.functions_with_logging / total_stats.functions * 100) if total_stats.functions > 0 else 0
    ...
    print(f"Functions with logging: {total_stats.functions_with_logging}/{total_stats.functions} ({total_stats.function_coverage_percent:.1f}%)")
