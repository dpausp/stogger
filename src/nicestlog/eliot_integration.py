...
class HumanReadableEliotDestination:
    ...
    def __init__(self, file: TextIO = None, show_timestamps: bool = True, 
                 show_task_ids: bool = False, max_width: int = 120):
    ...
    def _handle_action_start(self, message: Dict[str, Any], task_id: str):
        ...
        system_fields = {"action_type", "action_status", "task_uuid", "task_level", "timestamp"}
        params = {k: v for k, v in message.items() if k not in system_fields}
        
        self.file.write(f"{timestamp}{indent}{BLUE}▶{RESET_ALL} {BRIGHT}{action_type}{RESET_ALL}")
        
        if params:
            param_str = " ".join(f"{CYAN}{k}{RESET_ALL}={MAGENTA}{repr(v)}{RESET_ALL}" 
    ...
    def _handle_action_result(self, message: Dict[str, Any], task_id: str):
        ...
        # Extract result data
        result_data = {k: v for k, v in message.items() 
                      if k not in ["message_type", "action_type", "task_id", "task_level", "timestamp"]}
        
        self.file.write(f"{timestamp}{indent}{GREEN}✓{RESET_ALL} {DIM}{action_name}{RESET_ALL}")
        
        if result_data:
            result_str = " ".join(f"{CYAN}{k}{RESET_ALL}={MAGENTA}{repr(v)}{RESET_ALL}" 
    ...
    def _handle_action_failure(self, message: Dict[str, Any], task_id: str):
        ...
        self.file.write(f"{timestamp}{indent}{RED}✗{RESET_ALL} {DIM}{action_name}{RESET_ALL} ")
    ...
    def _handle_regular_message(self, message: Dict[str, Any], task_id: str):
        ...
        if msg_data:
            data_str = " ".join(f"{CYAN}{k}{RESET_ALL}={MAGENTA}{repr(v)}{RESET_ALL}" 
...
def setup_eliot_logging(destination: TextIO = None, human_readable: bool = True,
                       show_timestamps: bool = True, show_task_ids: bool = False) -> bool:
    """
    ...
    if not ELIOT_AVAILABLE:
        print("Warning: Eliot is not available. Install with: pip install eliot", file=sys.stderr)
        return False
