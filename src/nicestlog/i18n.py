...
class NicestlogTranslator:
    ...
    def _load_translations(self):
        ...
        if fallback_file.exists() and toml:
            try:
                ...
                log.warning("failed_to_load_fallback", file=str(fallback_file), error=str(e))
        ...
        if lang_file.exists() and toml:
            try:
                ...
                log.error("failed_to_load_translations", 
                         language=self.language,
                         file=str(lang_file), 
                         error=str(e))
        else:
            log.warning("translation_file_not_found", 
                       language=self.language,
                       file=str(lang_file),
                       fallback="en")
    ...
    def get(self, key: str, section: str = "general", **kwargs) -> str:
        ...
        """
        log.debug("getting_translation", key=key, section=section, language=self.language)
        ...
        if not translation:
            ...
            log.warning("translation_missing", key=key, section=section, language=self.language)
        ...
        try:
            ...
            log.error("translation_formatting_failed", 
                     key=key, 
    ...
    def _get_from_dict(self, translations: Dict[str, Any], section: str, key: str) -> Optional[str]:
...
def demo_translations():
    ...
    for lang in languages:
        ...
        print(f"   Error: {translator.get('file_not_found', 'errors', filename='test.log')}")
