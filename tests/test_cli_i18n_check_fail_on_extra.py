...
def test_cli_fail_on_extra_in_list_and_full(tmp_path: Path):
    ...
    r = run_cli([str(src), "--translation-dir", str(trans), "-l", "en", "--list-missing", "--fail-on-extra"], cwd=str(Path.cwd()))
    assert r.returncode == 1
    assert r.stdout.strip() == ""

    # full report: should mention extra keys and return 1 when --fail-on-extra
    r2 = run_cli([str(src), "--translation-dir", str(trans), "-l", "en", "--fail-on-extra"], cwd=str(Path.cwd()))
