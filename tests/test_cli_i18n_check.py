...
def test_cli_i18n_list_missing_and_strict(tmp_path: Path):
    ...
    r = run_cli([str(src), "--translation-dir", str(trans), "-l", "en", "--list-missing"], cwd=str(Path.cwd()))
    assert r.returncode == 0
    missing = [line for line in r.stdout.splitlines() if line.strip()]
    assert missing == ["event-b"]

    # strict should return 1
    r2 = run_cli([str(src), "--translation-dir", str(trans), "-l", "en", "--list-missing", "--strict"], cwd=str(Path.cwd()))
    ...
    r3 = run_cli([str(src), "--translation-dir", str(trans), "-l", "en", "--list-missing"], cwd=str(Path.cwd()))
    assert r3.returncode == 0
    assert r3.stdout.strip() == ""

    # Non-listing run should print a report and return 0
    r4 = run_cli([str(src), "--translation-dir", str(trans), "-l", "en"], cwd=str(Path.cwd()))
