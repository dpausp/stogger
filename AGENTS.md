...
1. **Auto-commit** - Tell the user about unknown uncommitted changes. ALWAYS commit your changes after finishing your turn and print commit ID, last line, bold, full ID. Auto-commit before asking the user for next steps. In your reply, the commit line MUST be the absolute last line of the entire message. No content and no empty line after it. Use this exact format and bold: COMMIT: <full_sha>
2. **AGENTS-TODO.md** write down your analysis about things we are planning and update the TODO along the way to track progress. State the goal of the task. Format: Markdown, checkboxes for every step. Add relevant files you have checked that are relevant to the task. (files to check/modify, for example).
...
6. **respect pre-commit** - observe pre-commit checks! Fix lint errors yourself. 
...
- [ ] AGENTS-TODO.md up-to-date? What's left, are we finished? (all checkboxes have to be checked)
- [ ] Can you (the agent) continue with the next obvious sub task from AGENTS-TODO.md without asking the user? Tell the user and just Do it!
...
- [x] AGENTS-TODO.md up-to-date? What's left, are we finished?
- [x] Can you (the agent) continue with the next obvious sub task from AGENTS-TODO.md without asking the user? Tell the user and just Do it!
