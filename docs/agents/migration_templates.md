...
This document provides ready-to-use templates for common stogger migration scenarios.
...
stoggertools migrate . --type print-to-structlog --backup --dry-run
...
stoggertools migrate . --json | jq '.recommendation.strategy'
# Should return: "logging-to-structlog"

# 2. Interactive migration (recommended for logging)
stoggertools migrate . --type logging-to-structlog --interactive --backup
...
# 4. Initialize stogger in main module
cat > init_logging.py << 'EOF'
import stogger
stogger.init_logging(verbose=True, syslog_identifier="your-project")
EOF

# 5. Validate and fix remaining issues
stoggertools check . --ast --fix
```
...
### Use Case
Projects already using structlog but need stogger improvements.
...
# 2. Add stogger and enhance existing logging
stoggertools check . --ast --fix --verbose
...
stoggertools review . --format json | jq '.overall_score'
```
...
cat > main.py << 'EOF'
#!/usr/bin/env python3
"""Main application with stogger integration."""
...
### Use Case
Adding stogger quality checks to continuous integration.
  ...
  logging-quality:
    ...
    steps:
      ...
      - name: Check logging quality
        run: |
          uv run stoggertools check . --ast --verbose
      ...
      - name: Analyze project structure
        run: |
          uv run stoggertools migrate . --json > analysis.json
...
repos:
  - repo: local
    hooks:
      - id: stogger-check
        name: stogger quality check
        entry: uv run stoggertools check . --ast
        language: system
        types: [python]
        
      - id: stogger-i18n
        name: stogger translation check
        entry: uv run stoggertools tools i18n check . --translation-dir translations -l en --strict
        language: system
...
stoggertools migrate . --json | jq '.complexity'
