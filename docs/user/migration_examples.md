...
stoggertools migrate . --no-dry-run --type print-to-structlog --backup
```
...
stoggertools migrate . --no-dry-run --type logging-to-structlog --backup
```
...
|---|---|---|
| **Print to Structlog** | `stoggertools migrate . --no-dry-run --type print-to-structlog` | Convert print() statements |
| **Logging to Structlog** | `stoggertools migrate . --no-dry-run --type logging-to-structlog --interactive` | Convert standard logging |
| **Format Strings** | `stoggertools migrate . --no-dry-run --type format-strings` | Convert f-strings to structured |
| **Analysis Only** | `stoggertools migrate .` | Analyze without changes |
| **Interactive Mode** | `stoggertools migrate . --no-dry-run --interactive` | Review each change |
| **With Backup** | `stoggertools migrate . --no-dry-run --backup` | Create backup files |
...
4. **Apply Migration**: Add `--no-dry-run` flag when ready
5. **Validate Results**: Use `stoggertools check` and `stoggertools demo`

For more examples and advanced patterns, see:
- [Logging Patterns](logging_patterns.md)
- [Advanced Features](../features/advanced_assistant.md)
